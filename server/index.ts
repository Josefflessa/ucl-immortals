...
// Import game engine functions dynamically or relatively
import {
...
interface RoomState {
  ...
  draftState: {
    round: number;
    turnIndex: number;
    draftOrder: string[]; // sequence of RoomPlayer ids (e.g. "player_0")
    ...
    currentOptionsByPlayer: Record<string, Player[]>; // player.id -> 6 cards
  };
}
      ...
      rooms.forEach((room: RoomState, code: string) => {
        ...
        if (idx !== -1) {
          ...
          if (room.phase === 'lobby') {
            ...
            // We just don't do anything, their socketId will be updated when they rejoin.
          }
        }
      });
    });
